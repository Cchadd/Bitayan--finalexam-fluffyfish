...
fish.image.src = 'assets/fish_sprite.png'; // Adjust the path as necessary

let pipes = [];

// Load background and pipe images
const backgroundImg = new Image();
backgroundImg.src = 'assets/backround.png';
...
function draw() {
  ...
  for (let i = 0; i < pipes.length; i++) {
    ...
    ctx.save();
    ctx.translate(p.x, p.height + p.gap);
    ctx.scale(1, -1);
    ctx.drawImage(pipeImg, 0, - (canvas.height - p.height - p.gap), p.width, canvas.height - p.height - p.gap);
    ctx.restore();
